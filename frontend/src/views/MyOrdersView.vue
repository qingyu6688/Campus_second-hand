        ...
        >
          ...
          <div class="flex justify-between items-center mb-3 pb-3 border-b border-gray-50">
             <div class="flex items-center space-x-2">
                ...
                <span class="text-xs font-bold text-gray-700">{{ item.seller?.nickname || '卖家' }}</span>
          ...
          <div class="flex justify-end space-x-2">
             ...
             <button v-if="item.status === 1" class="px-3 py-1.5 rounded-full bg-teal-50 text-teal-600 text-xs font-medium border border-teal-100">
                评价
             </button>
          </div>
        </div>

        <van-empty v-if="finished && orderList.length === 0" description="您还没有下单哦" />
      </van-list>
    </van-pull-refresh>
  </div>
</template>
